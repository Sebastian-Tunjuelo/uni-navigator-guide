...
export class CampusService {
  ...
  static async getBuildings(): Promise<Building[]> {
    ...
    if (error) {
      logger.error(`Campus buildings error: ${error.message}`);
  ...
  static async getBuildingsByCategory(
    ...
    if (error) {
      logger.error(`Campus category error: ${error.message}`);
      throw new Error(`Failed to fetch buildings by category: ${error.message}`);
    }
  ...
  static async getRoutes(): Promise<Route[]> {
    ...
    if (error) {
      logger.error(`Campus routes error: ${error.message}`);
   ...
   */
  static async getRouteBetweenBuildings(fromId: string, toId: string): Promise<Route | null> {
   ...
   */
  static async addBookmark(userId: string, buildingId: string): Promise<UserBookmark> {
    ...
    if (error) {
      logger.error(`Campus bookmark error: ${error.message}`);
   ...
   */
  static async removeBookmark(userId: string, buildingId: string): Promise<void> {
    logger.info(`Campus: User ${userId} removing bookmark for building ${buildingId}`);
    ...
    if (error) {
      logger.error(`Campus unbookmark error: ${error.message}`);
  ...
  static async getUserBookmarks(userId: string): Promise<Building[]> {
    ...
    if (error) {
      logger.error(`Campus bookmarks error: ${error.message}`);
    ...
    if (buildingError) {
      logger.error(`Campus building fetch error: ${buildingError.message}`);
      throw new Error(`Failed to fetch bookmarked buildings: ${buildingError.message}`);
    }
  ...
  static async getBuildingsNearby(
    ...
    if (error) {
      logger.error(`Campus nearby error: ${error.message}`);
      throw new Error(`Failed to fetch nearby buildings: ${error.message}`);
    }
