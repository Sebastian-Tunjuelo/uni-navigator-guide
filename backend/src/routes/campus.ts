 ...
 * Get all buildings
 */
router.get('/buildings', optionalAuth, async (req: Request, res: Response, next) => {
  try {
 ...
 * Get building by ID
 */
router.get('/buildings/:id', optionalAuth, async (req: Request, res: Response, next) => {
  try {
...
router.get(
  ...
  async (req: Request, res: Response, next) => {
    try {
      let category = Array.isArray(req.params.category) ? req.params.category[0] : req.params.category;

      if (
        !['academic', 'service', 'residence', 'sport'].includes(
          category as any
        )
      ) {
 ...
 */
router.get('/routes/:from/:to', optionalAuth, async (req: Request, res: Response, next) => {
  try {
 ...
 */
router.get('/nearby', optionalAuth, async (req: Request, res: Response, next) => {
  try {
    ...
    if (!lat || !lng) {
      throw new ValidationError('latitude (lat) and longitude (lng) required');
    }
 ...
 */
router.post('/bookmark', requireAuth, async (req: Request, res: Response, next) => {
  try {
 ...
 */
router.delete('/bookmark/:buildingId', requireAuth, async (req: Request, res: Response, next) => {
  try {
    const buildingId = Array.isArray(req.params.buildingId) ? req.params.buildingId[0] : req.params.buildingId;
    const userId = req.user?.id;
 ...
 */
router.get('/bookmarks', requireAuth, async (req: Request, res: Response, next) => {
  try {
    const userId = req.user?.id;
